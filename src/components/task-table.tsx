...
import {
  parseAiTodoRemark,
  parsePromptExecutionRemark,
  extractStoragePath,
  isAiResultRemark,
} from '@/lib/aiTodoProtocols';
...
const RemarkDisplay = ({
  ...
  // Prompt Execution remarks: "[prompt-execution|STATUS] ..."
  const promptExecution = parsePromptExecutionRemark(text);
  if (
    promptExecution.status === 'running' ||
    promptExecution.status === 'completed' ||
    promptExecution.status === 'failed'
  ) {
    let content = promptExecution.rest;
    const storagePath = extractStoragePath(content) || '';
    if (storagePath) {
      // Strip storage link from visible text; matches previous behavior.
      content = content
        .replace(/\[View results\]\(storage:\/\/([^)]+)\)/, '')
        .trim();
    }

    const status = promptExecution.status;
    const statusPill = (
      <span
        className={`capitalize px-1.5 py-0.5 text-xs rounded-full font-medium ${
          status === 'running'
    ...
    const handleViewReport = () => {
      ...
      window.dispatchEvent(
        new CustomEvent('view-report', { detail: storagePath }),
      );
    };
    ...
    return (
      <div
        id={`remark-${id}`}
        className="p-2 mt-1 rounded-lg border border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/20"
      >
        <div className="flex items-start gap-2.5">
          <CornerDownRight className="h-4 w-4 mt-0.5 text-purple-600 dark:text-purple-400 flex-shrink-0" />
          <div className="flex-1">
            <div className="flex justify-between items-center mb-1">
              <h4 className="text-xs font-semibold tracking-wider uppercase text-purple-700 dark:text-purple-400">
                Prompt Execution
              </h4>
              {statusPill}
            </div>
            ...
            {storagePath && (
              <Button
                variant="link"
                className="p-0 h-auto text-sm font-medium text-primary hover:underline mt-2"
  ...
  // AI To-Do remarks: "[ai-todo|STATUS] ..."
  const aiTodo = parseAiTodoRemark(text);
  if (aiTodo.status) {
    const status = aiTodo.status;
    const todoText = aiTodo.rest;
    ...
    return (
      <div
        id={`remark-${id}`}
        className={`p-2 mt-1 rounded-lg border ${statusConfig.borderColor} ${statusConfig.bgColor}`}
      >
        <div className="flex items-start gap-2.5">
          <StatusIcon
            className={`h-4 w-4 mt-0.5 ${statusConfig.iconColor} flex-shrink-0 ${
          ...
          <div className="flex-1">
            <div className="flex justify-between items-center mb-1">
              ...
              <span
                className={`capitalize px-1.5 py-0.5 text-xs rounded-full font-medium ${statusConfig.color}`}
              >
                {status}
              </span>
            </div>
            ...
            {showRunButton && (
              <div className="mt-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => onExecuteAiTodo(task, remark)}
                  disabled={isTaskBusy || !isOwner}
                >
  ...
  // AI Result remarks: "AI execution complete. [View results](storage://...)"
  const summaryMatch = text.match(/\*\*Summary:\*\*\n([\s\S]+)/); // NOTE: uses [\s\S] instead of /s flag (no /s) to support current TS target
  const storagePathForResult = extractStoragePath(text);
  if (storagePathForResult && isAiResultRemark(text)) {
    const summary = summaryMatch ? summaryMatch[1].trim() : '';

    const handleViewReport = () => {
      window.dispatchEvent(
        new CustomEvent('view-report', { detail: storagePathForResult }),
      );
    };
    ...
    return (
      <div
        id={`remark-${id}`}
        className={`p-2 mt-1 rounded-lg border ${resultBorderColor} ${resultBgColor}`}
      >
        <div className="flex items-start gap-2.5">
          <CheckCircle2
            className={`h-4 w-4 mt-0.5 ${resultIconColor} flex-shrink-0`}
          />
        </div>
        <div className="flex-1">
          <h4
            className={`text-xs font-semibold tracking-wider uppercase ${resultHeaderColor}`}
          >
            AI Execution Complete
          </h4>
          {summary && (
            <p className="text-sm text-foreground/90 mt-1 mb-2">{summary}</p>
          )}
          <div className="flex items-center gap-2 flex-wrap mt-2">
            ...
            {promptGenMatch && (
              <Button
                ...
                onClick={() => onRunRefinedPrompt(task, remark)}
                disabled={isTaskBusy || !isOwner}
              >
...
export function TaskTable({
  ...
  const handleUpdateTask = (taskToUpdate: Task) => {
    ...
    onUpdate({ id: checklist.id, tasks: newTasks });
  };
  ...
  const handleDeleteTask = (taskId: string) => {
    if (
      ...
      const newTasks = checklist.tasks.filter(
        (t) => t.id !== taskId,
      );
      onUpdate({ id: checklist.id, tasks: newTasks });
    }
  ...
  const handleTaskCompletionChange = (
    ...
    const systemRemarkTextIncomplete =
    ...
    if (isComplete) {
      ...
      const completionRemarkIndex =
        updatedRemarks.findIndex(
          (r) =>
            r.text === systemRemarkText &&
            r.userId === 'system',
        );

      if (completionRemarkIndex > -1) {
        const completionRemark =
          updatedRemarks[completionRemarkIndex];
        const remarkDate = parseISO(
          completionRemark.timestamp,
        );
  ...
  const openRemarksSheet = (task: Task) => {
    const event = new CustomEvent('open-remarks', {
      detail: task,
    });
    window.dispatchEvent(event);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-4 items-center no-print">
        <h2 className="text-2xl font-bold font-headline text-foreground flex-shrink-0">
          {checklist.name}
        </h2>
          ...
          <TableBody>
            {filteredAndSortedTasks.length > 0 ? (
              filteredAndSortedTasks.map((task) => {
                const isTaskBusy =
                  task.remarks.some((r) =>
                    runningRemarkIds.includes(r.id),
                  );
                const flattenedRemarks =
                  buildFlattenedRemarks(task.remarks);
      ...
      <div className="md:hidden space-y-4">
        {filteredAndSortedTasks.length > 0 ? (
          filteredAndSortedTasks.map((task) => {
            const isTaskBusy = task.remarks.some(
              (r) => runningRemarkIds.includes(r.id),
            );
            const flattenedRemarks =
              buildFlattenedRemarks(task.remarks);
