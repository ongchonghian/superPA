...
From your analysis of each task, generate one or more *new* suggestions for AI To-Dos.
- If multiple distinct automatable actions can be derived from a single task's context, you MUST generate a separate suggestion for each one.
...
Your final output MUST be a single JSON object containing a 'suggestions' array. This array must be a flat list containing ALL suggestions you identified across ALL tasks. If you find no new automatable actions for a specific task, simply move on to the next one. If, after analyzing all tasks, you have found no suggestions at all, return an empty 'suggestions' array.
